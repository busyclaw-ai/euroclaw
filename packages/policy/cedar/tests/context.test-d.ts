...
// this file is the executable spec for the request context Cedar declares (`{ principal }`) being
// folded onto, and required by, governed calls.
import { createGovernance } from "@euroclaw/core";
import { describe, test } from "vitest";
import { cedar } from "../src/index";

describe("cedar request-context typing", () => {
	test("a cedar-governed core requires a string `principal`", () => {
	...
	test("the approval flag is server-derived — spoofing type-checks but is a runtime no-op", () => {
		...
		// here — but a spoofed flag never bypasses approval: cedar re-derives it at runtime (see
		// cedar.test.ts, "a confirm-gated policy cannot be satisfied by caller context"). NOT an error.
		void governed.handleToolCall(
			{ name: "x", args: {} },
